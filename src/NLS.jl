...
function dfp(initial_values::Function, T, dt, dx; lambda=-2, a=-30, b=30)
	...
	J = ceil(Int64, (b-a) / dx) # compute the number of cells
	dx = (b-a) / J 
	...
	A = spdiagm(0  => 2 * ones(matrix_size),  # main diagonal
		    -1 => -ones(matrix_size - 1), # lower minor diagonal
		    1  => ones(matrix_size -1 ))  # upper minor diagonal
	...
	x = collect(range(a, stop=b, length=matrix_size))
	U0 = initial_values.(x)
	...
	for k in 1:K
		...
		iterations = 0
		while error > eps
			Up1 = FMp1 \ (Mk * U[:, k] .- F(Up, U[:, k], lambda))
			error = norm(Up1 .- Up)
			Up = Up1
